...
function formatDate(d: Date): string {
   ...
   const formattedDay = day + (day % 10 == 1 ? 'st' : (day % 10 == 2 ? 'nd' : (day % 10 == 3 ? 'rd': 'th')))
   return monthNames[month] + ' ' + formattedDay + (year != 2023 ? ' \'' + year.toString().slice(-2) : '')
}
...
export default function StreamRow({user, log}: {user: boolean, log: Log}){
   ...
   const remainingFormatted = data ? Number(data[0].result) / (10 ** (token == 'USDC' ? 6 : 18)) : 0
   const withdrawableFormatted = !withdrawable ? 0 : (Number(withdrawable) / (10 ** (token == 'USDC' ? 6 : 18))).toFixed(2)

   const withdrawnPct = 1 - (remainingFormatted / streamAmount)
   const roundedRemaining = Number((withdrawnPct).toFixed(1)) * 10
   let progressString = withdrawnPct == 0 ? `100% \xa0` : `${((1 - withdrawnPct) * 100).toFixed(1)}% `
   for(let i = 0; i < 10; i++){
      progressString += i < roundedRemaining ? '▓' : '░'
   }

   const timePct = (new Date().getTime() - (log.startTime * 1000)) / (log.stopTime * 1000 - log.startTime*1000)
   ...
   return(
      <div className='flex flex-row gap-x-2 items-center'>
         {!user && 
         <Link className='w-44 hover:underline text-gray-500' href={`https://etherscan.io/address/${recipient}`} target="_blank">
         ...
         <Link className='w-24 hover:underline text-gray-500' href={`https://etherscan.io/address/${stream}`} target="_blank">
            Contract
         </Link>
      </div>
   )
