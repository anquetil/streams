...
export default function UserStreams({logs}: {logs: Log[]}){
   ...
   const testAddress = '0xD1EAeFBeFFD4106A1A166CD26a1Fe23140D6a42e'
   let userLogs:Log[] = []
   if(isConnected){
      userLogs = logs.filter((l) => (l.recipient.toLowerCase() == testAddress?.toLowerCase()))
   }
         ...
         </div>
         {
            isConnected ? (
               ...
               ? <div className="text-gray-600">No streams for this address.</div> :
                  userLogs.map((l, i) => (
                     <StreamRow key={i} log={l} user={true} />
                  ))
            ) : <CustomConnectButton />
         }
      </div>
