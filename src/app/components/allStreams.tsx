...
const getData = cache(async () => {
   ...
   const logs = await client.getLogs({
      ...
      event: parseAbiItem('event StreamCreated(address indexed msgSender, address indexed payer, address indexed recipient, uint256 tokenAmount, address tokenAddress, uint256 startTime, uint256 stopTime, address streamAddress)'),
   ...
   return logs.map((l) => {
      return ({
         ...
         token: l.args.tokenAddress == '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48' ? 'USDC' : 'WETH' as 'USDC' | 'WETH',
   ...
   if(logs && logs.length > 0 ){
      return (
         <div className='px-12 py-6'>
            ...
            {logs.map((l, i) => {
               return (
                  <StreamRow key={i} log={l} user={false} />
               )
            })}
         </div>
      )
